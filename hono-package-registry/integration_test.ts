...
Deno.test("integration: self-host -- this repo via git store -> deno uses ABC package", async () => {
  ...
  writeDenoJsonWithImport(projectDir, "@publicdomainrelay/package-store-abc", registry.url, abcVersion!);
  await Deno.writeTextFile(join(projectDir, "main.ts"), `
import type { PackageStore, PackageEntry, PackageVersion } from "@publicdomainrelay/package-store-abc";
